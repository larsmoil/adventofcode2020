...
impl Policy {
    pub fn new(policy_and_password: &str, policy_type: PolicyType) -> Policy {
        let mut policy_and_password = policy_and_password.split(":");
        let policy = policy_and_password.next().unwrap().to_string();
    ...
    pub fn is_valid(&self) -> bool {
        match &self.policy_type {
            PolicyType::ExactlyOnePositionMatches => {
                let (pos1, pos2, character) = Policy::parse(&self.policy);
                let chars = &self.password.chars().collect::<Vec<char>>();
                let pos1_match = chars[pos1 - 1] == character;
                let pos2_match = chars[pos2 - 1] == character;
                pos1_match ^ pos2_match
            },
            PolicyType::MinMaxOccurrences => {
                let (min_occurrences, max_occurrences, character) = Policy::parse(&self.policy);
                let num_occurrences = &self.password.chars().filter(|&c| c == character).count();
                RangeInclusive::new(min_occurrences, max_occurrences).contains(num_occurrences)
            },
        }
    }

    fn parse(policy: &str) -> (usize, usize, char) {
        let policy = policy.split(" ").map(|s| s.to_string()).collect::<Vec<String>>();
        let mut numbers = policy[0].split("-").map(|n| n.parse::<usize>().unwrap());
        let character = policy[1].chars().next().unwrap();
        let first_number = numbers.next().unwrap();
        let second_number = numbers.next().unwrap();

        return (first_number, second_number, character);
    }
}

pub fn input() -> std::vec::Vec<String> {
    return vec![
        "6-8 s: svsssszslpsp",
        "3-4 n: gncn",
...
mod tests {
    use super::*;
    use crate::passwords::PolicyType::{MinMaxOccurrences, ExactlyOnePositionMatches};

    #[test]
    fn test_pt1_example() {
        assert_eq!(Policy::new("1-3 a: abcde", MinMaxOccurrences).is_valid(), true);
        assert_eq!(Policy::new("1-3 a: abcde", MinMaxOccurrences).password, "abcde");
        assert_eq!(Policy::new("1-3 a: abcde", MinMaxOccurrences).policy, "1-3 a");
        assert_eq!(Policy::new("1-3 b: cdefg", MinMaxOccurrences).is_valid(), false);
        assert_eq!(Policy::new("1-3 b: cdefg", MinMaxOccurrences).password, "cdefg");
        assert_eq!(Policy::new("1-3 b: cdefg", MinMaxOccurrences).policy, "1-3 b");
        assert_eq!(Policy::new("2-9 c: ccccccccc", MinMaxOccurrences).is_valid(), true);
        assert_eq!(Policy::new("2-9 c: ccccccccc", MinMaxOccurrences).password, "ccccccccc");
        assert_eq!(Policy::new("2-9 c: ccccccccc", MinMaxOccurrences).policy, "2-9 c");
    }

    #[test]
    fn test_pt2_example() {
        assert_eq!(Policy::new("1-3 a: abcde", ExactlyOnePositionMatches).is_valid(), true);
        assert_eq!(Policy::new("1-3 a: abcde", ExactlyOnePositionMatches).password, "abcde");
        assert_eq!(Policy::new("1-3 a: abcde", ExactlyOnePositionMatches).policy, "1-3 a");
        assert_eq!(Policy::new("1-3 b: cdefg", ExactlyOnePositionMatches).is_valid(), false);
        assert_eq!(Policy::new("1-3 b: cdefg", ExactlyOnePositionMatches).password, "cdefg");
        assert_eq!(Policy::new("1-3 b: cdefg", ExactlyOnePositionMatches).policy, "1-3 b");
        assert_eq!(Policy::new("2-9 c: ccccccccc", ExactlyOnePositionMatches).is_valid(), false);
        assert_eq!(Policy::new("2-9 c: ccccccccc", ExactlyOnePositionMatches).password, "ccccccccc");
        assert_eq!(Policy::new("2-9 c: ccccccccc", ExactlyOnePositionMatches).policy, "2-9 c");
    }
}
