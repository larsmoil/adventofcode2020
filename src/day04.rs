...
fn validate(passport: String, field_rules: &HashMap<&str, fn(&str) -> bool>) -> bool {
    let mut entries = passport
        .split(" ")
        .map(|entry| {
    ...
    for (field, value) in entries {
        let required_field = required_fields.iter().enumerate().find(|(_, &required_field)| required_field == field);

        if required_field.is_some() {
            required_fields.remove(required_field.unwrap().0);
        }

        if field_rules.contains_key(field) && !field_rules[field](value) {
            return false;
        }
    }
...
pub fn pt1(input: String) -> i32 {
    ...
    passports
        .map(|passport| validate(passport, &HashMap::new()))
        .map(|b| if b { 1 } else { 0 })
        .fold(0, |a, b| a + b)
}
...
pub fn pt2(input: String) -> usize {
    let passports = input.split("\n\n")
        .map(|passport| passport.replace("\n", " "));
    let mut field_rules: HashMap<&str, fn(&str) -> bool> = HashMap::new();
    field_rules.insert("byr", |value| {
        let birth_year = value.parse::<i32>().unwrap();
        1920 <= birth_year && birth_year <= 2002
    });
    field_rules.insert("iyr", |value| {
        let issue_year = value.parse::<i32>().unwrap();
        2010 <= issue_year && issue_year <= 2020
    });
    field_rules.insert("eyr", |value| {
        let expiration_year = value.parse::<i32>().unwrap();
        2020 <= expiration_year && expiration_year <= 2030
    });
    field_rules.insert("hgt", |value| {
        return match value.strip_suffix("cm") {
            None => false,
            Some(centimeters) => RangeInclusive::new(150, 193).contains(&centimeters.parse::<i32>().unwrap())
        } || match value.strip_suffix("in") {
            None => false,
            Some(inches) => RangeInclusive::new(59, 79).contains(&inches.parse::<i32>().unwrap())
        }
    });
    field_rules.insert("hcl", |value| {
        match value.strip_prefix("#") {
            None => false,
            Some(s) => s.chars().all(|c| c.is_ascii_hexdigit())
        }
    });
    field_rules.insert("ecl", |value| {
        vec!["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].contains(&value)
    });
    field_rules.insert("pid", |value| {
        value.chars().all(char::is_numeric) && value.len() == 9
    });
    passports
        .map(|passport| validate(passport, &field_rules))
        .map(|b| if b { 1 } else { 0 })
        .fold(0, |a, b| a + b)
}
...
pid:#725759
hcl:#602927 iyr:2013 byr:2003 eyr:2023 cid:100".to_string()
}
...
iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719".to_string()
    ...
    #[test]
    fn test_pt1_example() {
        assert_eq!(pt1(example_input()), 2);
    }

    #[test]
    fn test_pt1() {
        assert_eq!(pt1(input()), 256);
    }

    #[test]
    fn test_pt2_example() {
        assert_eq!(pt2(example_input2()), 4);
    }

    #[test]
    fn test_pt2() {
        assert_eq!(pt2(input()), 198);
    }
}
